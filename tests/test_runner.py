...
class TestLoadDataset:
    ...
    def test_load_csv_with_custom_columns(self, tmp_path, full_config):
        ...
        df = pd.DataFrame({"X": ["a", "b", "c"], "y": [1, 0, 1], "group": ["A", "B", "A"]})
...
class TestCalculateMetrics:
    ...
    def test_calculate_metrics_with_biased_data(self, full_config, biased_data):
        ...
        results = calculate_metrics(full_config, biased_data["sensitive"], biased_data["y_pred"], biased_data["y_true"])
    ...
    def test_run_fairness_check_with_custom_thresholds(self, tmp_path, temp_csv_file):
        ...
        config = Config(
            ...
            fairness=FairnessConfig(
                demographic_parity_threshold=0.5, equal_opportunity_threshold=0.5  # High threshold
            ),
        )
